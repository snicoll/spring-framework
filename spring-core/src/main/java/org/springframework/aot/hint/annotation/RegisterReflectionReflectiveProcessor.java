 ...
 * implementation for custom annotation using {@link RegisterReflection}.
 *
...
public class RegisterReflectionReflectiveProcessor implements ReflectiveProcessor {
	...
	private static final BindingReflectionHintsRegistrar dataBindingRegistrar = new BindingReflectionHintsRegistrar();
	...
	public void registerReflectionHints(ReflectionHints hints, AnnotatedElement element) {
		...
		for (RegisterReflection annotation : annotations) {
			...
			switch (annotation.strategy()) {
				case SIMPLE -> registerSimpleReflectionHints(hints, registration);
				case DATA_BINDING ->
						registerDataBindingReflectionHints(hints, registration);
			}
		}
	}
	...
	protected void registerSimpleReflectionHints(ReflectionHints hints, ReflectionRegistration registration) {
		for (Class<?> target : registration.classes) {
			hints.registerType(target, type -> type.withMembers(registration.memberCategories));
		}
	}

	protected void registerDataBindingReflectionHints(ReflectionHints hints, ReflectionRegistration registration) {
		dataBindingRegistrar.registerReflectionHints(hints, registration.classes);
	}
