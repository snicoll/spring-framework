...
/**
 * Default {@link MethodReference2} implementation based on a {@link MethodSpec}.
 *
 * @author Stephane Nicoll
 * @since 6.0
 */
public class DefaultMethodReference implements MethodReference2 {
	...
	public CodeBlock toInvokeCodeBlock(ClassName declaringClass, ArgumentCodeGenerator argumentCodeGenerator) {
		String methodName = this.method.name;
		CodeBlock.Builder code = CodeBlock.builder();
		if (isStatic()) {
			Assert.notNull(this.declaringClass, "static method reference must define a declaring class");
			if (isSameDeclaringClass(declaringClass)) {
