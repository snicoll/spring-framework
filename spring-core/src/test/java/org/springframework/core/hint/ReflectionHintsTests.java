...
class ReflectionHintsTests {
	...
	void registerType() {
		...
		assertThat(this.reflectionHints.typeHints()).singleElement().satisfies(typeOnlyHint(String.class));
	}

	@Test
	void registerTypeReuseBuilder() {
		this.reflectionHints.registerType(TypeReference.of(String.class),
				typeHint -> typeHint.withMembers(MemberCategory.DECLARED_CONSTRUCTORS));
		...
		assertThat(this.reflectionHints.typeHints()).singleElement().satisfies(typeHint -> {
			...
			assertThat(typeHint.getMemberCategories()).containsOnly(MemberCategory.DECLARED_CONSTRUCTORS);
		});
	}
	...
	void registerClass() {
		...
		assertThat(this.reflectionHints.typeHints()).singleElement().satisfies(typeOnlyHint(Integer.class));
	}
	...
	private Consumer<TypeHint> typeOnlyHint(Class<?> type) {
		return typeHint -> {
