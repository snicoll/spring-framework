...
public class ImportAwareBeanFactoryContributionTests {
	...
	@Test
	void contributeWithNoImportAwareConfigurationReturnsEmptyContribution() {
		GeneratedTypeContext generationContext = mock(GeneratedTypeContext.class);
		BeanFactoryContribution contribution = createContribution(SimpleConfiguration.class);
		BeanFactoryInitialization initialization = new BeanFactoryInitialization(generationContext);
		contribution.applyTo(initialization);
		assertThat(initialization.toCodeBlock().isEmpty()).isTrue();
		verifyNoInteractions(generationContext);
	}
	...
	private BeanFactoryContribution createContribution(Class<?> type) {
		...
		BeanFactoryContribution contribution = pp.contribute(beanFactory);
