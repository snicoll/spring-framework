...
import org.springframework.context.GenericApplicationEvent;
...
public class ApplicationListenerMethodAdapterTests extends AbstractApplicationEventListenerTests {
	...
	public void listenerWithPayloadTypeErasure() { // Always accept such event when the type is unknown
		Method method = ReflectionUtils.findMethod(SampleEvents.class,
				"handleString", String.class);
		supportsEventType(true, method, ResolvableType.forClass(GenericApplicationEvent.class));
	}
	...
	public void invokeListenerCheckedException() {
		...
		thrown.expect(EventListenerExecutionException.class);
	...
	public void invokeListenerInvalidProxy() {
		...
		StaticApplicationListenerMethodAdapter listener =
				new StaticApplicationListenerMethodAdapter(this.context, method, bean);
		thrown.expect(IllegalStateException.class);
	...
	public void invokeListenerWithPayload() {
		Method method = ReflectionUtils.findMethod(SampleEvents.class,
				"handleString", String.class);
		GenericApplicationEvent<String> event = new GenericApplicationEvent<>(this, "test");
		invokeListener(method, event);
	...
	public void invokeListenerWithPayloadWrongType() {
		Method method = ReflectionUtils.findMethod(SampleEvents.class,
				"handleString", String.class);
		GenericApplicationEvent<Long> event = new GenericApplicationEvent<>(this, 123L);
		invokeListener(method, event);
	...
	public void beanInstanceRetrievedAtEveryInvocation() {
		...
		ApplicationListener<ApplicationEvent> listener = new ApplicationListenerMethodAdapter(
				this.context, "testBean", GenericTestEvent.class, method);
		GenericTestEvent<String> event = createGenericTestEvent("test");
	...
	private ApplicationListenerMethodAdapter createTestInstance(Method method) {
		return new StaticApplicationListenerMethodAdapter(this.context, method, this.sampleEvents);
	}

	private ResolvableType createGenericEventType(Class<?> payloadType) {
		return ResolvableType.forClassWithGenerics(GenericApplicationEvent.class, payloadType);
	}
	...
	private static class StaticApplicationListenerMethodAdapter
		...
		public StaticApplicationListenerMethodAdapter(ApplicationContext applicationContext,
				Method method, Object targetBean) {
			super(applicationContext, "unused", targetBean.getClass(), method);
