 ...
 * @since 4.2.0
 */
public class ApplicationListenerMethodAdapter implements SmartApplicationListener {
	...
	protected Object[] resolveArguments(ApplicationEvent event) {
		if (!ApplicationEvent.class.isAssignableFrom(this.declaredEventType.getRawClass())
				&& event instanceof GenericApplicationEvent) {
			Object payload = ((GenericApplicationEvent) event).getPayload();
	...
	public boolean supportsEventType(ResolvableType eventType) {
		...
		else if (eventType.getRawClass().isAssignableFrom(GenericApplicationEvent.class)) {
			ResolvableType payloadType = eventType.getGeneric();
	...
	protected Object doInvoke(Object... args) {
		...
		try {
			...
			if (targetException instanceof RuntimeException) {
				...
				throw new EventListenerExecutionException(msg, targetException);
			}
		}
	}
