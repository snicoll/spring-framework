...
public abstract class ReflectiveProcessorAotContributionProvider {
	...
	/**
	 * Create an AOT contribution for the specified classes. The returned
	 * contribution inspect each class for the presence of {@link Reflective}
	 * and registers the necessary reflection hints as a result.
	 * @param classes the classes to inspect
	 * @return an AOT contribution for those classes or {@code null} if they aren't any
	 */
	@Nullable
	...
	public static BeanFactoryInitializationAotContribution from(Class<?>[] classes) {
		return (classes.length > 0 ? new AotContribution(classes) : null);
	}
	 ...
	 */
	@Nullable
	public static BeanFactoryInitializationAotContribution scan(ClassLoader classLoader, String... packageNames) {
	...
	private static class ReflectiveClassPathScanner extends ClassPathScanningCandidateComponentProvider {
		...
		ReflectiveClassPathScanner(ClassLoader classLoader) {
			super(false);
