...
public abstract class AbstractAdaptableMessageListener
	 ...
	 * @throws CouldNotSendReplyException if the response message could not be sent
	 * @see #buildMessage
	...
	protected void handleResult(Object result, Message request, Session session) {
		if (session != null) {
			...
			try {
				...
				throw new CouldNotSendReplyException("Failed to send reply with payload '" + result + "'", e);
			}
		}
		else {
