...
public class AnnotationHttpServiceRegistrarTests {
	...
	void basicListingWithAot() {
		...
		compile(applicationContext, (initializer, compiled) -> {
			...
			assertThat(registry.registeredClientTypes())
					.hasEntrySatisfying(ECHO_GROUP, clients -> assertThat(clients).containsOnly(EchoA.class, EchoB.class))
					.hasSize(1);
		});
	}
	...
	void basicScanWithAot() {
		...
		compile(applicationContext, (initializer, compiled) -> {
			...
			assertThat(registry.registeredClientTypes())
					.hasEntrySatisfying(ECHO_GROUP, clients -> assertThat(clients).containsOnly(EchoA.class, EchoB.class))
					.hasEntrySatisfying(GREETING_GROUP, clients -> assertThat(clients).containsOnly(GreetingA.class, GreetingB.class))
					.hasSize(2);
		});
	}
