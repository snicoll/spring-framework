...
/**
 * Tests for {@link JpaManagedTypesBeanRegistrationAotProcessor}.
 *
 * @author Stephane Nicoll
 */
class JpaManagedTypesBeanRegistrationAotProcessorTests {
	...
	void processEntityManagerWithPackagesToScan() {
		...
		compile(context, (initializer, compiled) -> {
			GenericApplicationContext freshApplicationContext = toFreshApplicationContext(
					initializer);
			JpaManagedTypes jpaManagedTypes = freshApplicationContext.getBean("jpaManagedTypes", JpaManagedTypes.class);
			assertThat(jpaManagedTypes).isInstanceOf(SimpleJpaManagedTypes.class);
			assertThat(jpaManagedTypes.getManagedClassNames()).containsExactlyInAnyOrder(
					DriversLicense.class.getName(), Person.class.getName());
			assertThat(jpaManagedTypes.getManagedPackages()).isEmpty();
	...
	public static class EntityManagerWithPackagesToScanConfiguration {
		...
		@Bean
		public JpaManagedTypes jpaManagedTypes(ResourcePatternResolver resourcePatternResolver) {
			this.scanningInvoked = true;
			return new JpaManagedTypesScanner(resourcePatternResolver)
					.scan("org.springframework.orm.jpa.domain");
		}

		@Bean
		public LocalContainerEntityManagerFactoryBean entityManagerFactory(DataSource dataSource,
				JpaVendorAdapter jpaVendorAdapter, JpaManagedTypes jpaManagedTypes) {
			LocalContainerEntityManagerFactoryBean entityManagerFactoryBean = new LocalContainerEntityManagerFactoryBean();
			entityManagerFactoryBean.setDataSource(dataSource);
			entityManagerFactoryBean.setJpaVendorAdapter(jpaVendorAdapter);
			entityManagerFactoryBean.setJpaManagedTypes(jpaManagedTypes);
