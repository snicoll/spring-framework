...
/**
 * Scanner of JPA managed types.
 *
 * @author Stephane Nicoll
 * @since 6.0
 */
public class JpaManagedTypesScanner {
	...
	public JpaManagedTypesScanner(ResourcePatternResolver resourcePatternResolver) {
		this.resourcePatternResolver = resourcePatternResolver;
		this.componentsIndex = CandidateComponentsIndexLoader.loadIndex(resourcePatternResolver.getClassLoader());
	}

	/**
	 * Scan the specified packages and return a {@link JpaManagedTypes} that
	 * represents the result of the scanning.
	 * @param packagesToScan the packages to scan
	 * @return the {@link JpaManagedTypes} instance
	 */
	public JpaManagedTypes scan(String... packagesToScan) {
			...
			for (Resource resource : resources) {
				try {
					...
					if (matchesFilter(reader, readerFactory)) {
						...
						scanResult.managedClassNames.add(className.substring(0,
	...
	private static class ScanResult {
		...
		JpaManagedTypes toJpaManagedTypes() {
			return new SimpleJpaManagedTypes(this.managedClassNames, this.managedPackages, this.persistenceUnitRootUrl);
		}
