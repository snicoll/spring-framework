...
import org.springframework.orm.jpa.JpaManagedTypes;
import org.springframework.orm.jpa.JpaManagedTypesScanner;
...
public class DefaultPersistenceUnitManager
	...
	@Nullable
	private JpaManagedTypes jpaManagedTypes;
	 ...
	 * <p>Consider setting a {@link JpaManagedTypes} instead that allows the scanning
	 * logic to be optimized by AOT processing.
	 ...
	 * @see #setJpaManagedTypes(JpaManagedTypes)
	...
	private List<SpringPersistenceUnitInfo> readPersistenceUnitInfos() {
		...
		boolean buildDefaultUnit = (this.jpaManagedTypes != null || this.packagesToScan != null || this.mappingResources != null);
			...
			if (foundDefaultUnit) {
				if (logger.isWarnEnabled()) {
					logger.warn("Found explicit default persistence unit with name '" + defaultName + "' in persistence.xml - " +
							"overriding local default persistence unit settings ('packagesToScan', 'mappingResources' or 'managedClassNames')");
				}
			}
			else {
	...
	private SpringPersistenceUnitInfo buildDefaultPersistenceUnitInfo() {
		...
		if (this.jpaManagedTypes != null) {
			applyJpaManagedTypes(scannedUnit, this.jpaManagedTypes);
		}
		else if (this.packagesToScan != null) {
			applyJpaManagedTypes(scannedUnit, new JpaManagedTypesScanner(
	...
	private void applyJpaManagedTypes(SpringPersistenceUnitInfo scannedUnit, JpaManagedTypes jpaManagedTypes) {
		jpaManagedTypes.getManagedClassNames()
				.forEach(scannedUnit::addManagedClassName);
		jpaManagedTypes.getManagedPackages()
				.forEach(scannedUnit::addManagedPackage);
		URL persistenceUnitRootUrl = jpaManagedTypes.getPersistenceUnitRootUrl();
