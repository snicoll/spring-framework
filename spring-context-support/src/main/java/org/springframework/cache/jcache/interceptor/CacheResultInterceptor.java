...
public class CacheResultInterceptor extends AbstractKeyCacheInterceptor<CacheResultOperation, CacheResult> {
	...
	@Override
	protected Object invoke(CacheOperationInvocationContext<CacheResultOperation> context, Invoker invoker) throws Throwable {
	 ...
	 */
	protected void checkForCachedException(Cache exceptionCache, Object cacheKey) throws Throwable {
	 ...
	 */
	static <T extends Throwable> T rewriteCallStack(T exception, String className, String methodName) {
		T clone = cloneException(exception);
		if (clone == null) {
			return exception;
		}
		...
		if (index == -1 || cachedIndex == -1) {
			return clone; // Cannot find common ancestor
		}
