...
public class TransactionalEventListenerTests {
	...
	public void regularTransaction() {
		load(ImmediateTestListener.class, BeforeCommitTestListener.class, AfterCompletionTestListener.class);
		this.transactionTemplate.execute(status -> {
			TransactionSynchronizationManager.registerSynchronization(new EventTransactionSynchronization(10) {
				@Override
				public void beforeCommit(boolean readOnly) {
	...
	public void noTransaction() {
		...
		this.eventCollector.assertEvents(EventCollector.IMMEDIATELY, "test");
	...
	static class BeforeCommitTestListener extends BaseTransactionalTestListener {
		...
		@TransactionalEventListener(phase = Phase.BEFORE_COMMIT)
		@Order(15)
	...
	@TransactionalEventListener(phase = Phase.AFTER_COMMIT, condition = "!'SKIP'.equals(#p0)")
