...
public class ApplicationListenerMethodTransactionalAdapterTests {
	...
	public void noAnnotation() {
		...
		ApplicationListenerMethodTransactionalAdapter.determinePhase(m);
	}
	...
	public void defaultPhase() {
		...
		assertPhase(m, Phase.AFTER_COMMIT);
	}

	@Test
	public void phaseSet() {
		Method m = ReflectionUtils.findMethod(PhaseConfigurationTestListener.class, "phaseSet", String.class);
		assertPhase(m, Phase.AFTER_ROLLBACK);
	}

	private void assertPhase(Method method, Phase expected) {
		assertNotNull("Method must not be null", method);
		Phase actual = ApplicationListenerMethodTransactionalAdapter.determinePhase(method);
		assertEquals("Wrong phase for '" + method + "'", expected, actual);
	}
	...
	static class PhaseConfigurationTestListener {
		...
		@TransactionalEventListener(phase = Phase.AFTER_ROLLBACK)
