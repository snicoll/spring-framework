...
class MockitoBeanOverrideProcessor implements BeanOverrideProcessor {
	...
	@Override
	public MockitoMetadata createMetadata(Field field, Annotation overrideAnnotation, ResolvableType typeToMock,
			Class<?> testClass) {
		if (overrideAnnotation instanceof MockitoBean mockBean) {
			return new MockitoBeanMetadata(mockBean, field, typeToMock);
		}
		else if (overrideAnnotation instanceof MockitoSpyBean spyBean) {
			return new MockitoSpyBeanMetadata(spyBean, field, typeToMock);
		}
