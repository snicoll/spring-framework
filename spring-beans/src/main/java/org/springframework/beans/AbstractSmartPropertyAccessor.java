 ...
 */
public abstract class AbstractSmartPropertyAccessor<A extends AbstractSmartPropertyAccessor<?>>
		extends AbstractPropertyAccessor {

	/**
	 * We'll create a lot of these objects, so we don't want a new logger every time.
	 */
	private static final Log logger = LogFactory.getLog(AbstractSmartPropertyAccessor.class);
	...
	static {
		try {
			javaUtilOptionalClass =
					ClassUtils.forName("java.util.Optional", AbstractSmartPropertyAccessor.class.getClassLoader());
		}
	 ...
	 */
	private Map<String, A> nestedPropertyAccessors;
	 ...
	 */
	public AbstractSmartPropertyAccessor() {
		this(true);
	}
	 ...
	 * @see #setWrappedInstance
	 */
	public AbstractSmartPropertyAccessor(boolean registerDefaultEditors) {
	 ...
	 */
	public AbstractSmartPropertyAccessor(Object object) {
		registerDefaultEditors();
	 ...
	 */
	public AbstractSmartPropertyAccessor(Class<?> clazz) {
		registerDefaultEditors();
	 ...
	 */
	public AbstractSmartPropertyAccessor(Object object, String nestedPath, Object rootObject) {
		registerDefaultEditors();
	 ...
	 */
	protected AbstractSmartPropertyAccessor(Object object, String nestedPath, A superBw) {
	...
	protected PropertyHandler getPropertyHandler(String propertyName) throws BeansException {
		...
		A nestedPa = getPropertyAccessorForPropertyPath(propertyName);
	...
	public TypeDescriptor getPropertyTypeDescriptor(String propertyName) throws BeansException {
		try {
			A nestedBw = getPropertyAccessorForPropertyPath(propertyName);
			String finalPath = getFinalPath(nestedBw, propertyName);
			PropertyTokenHolder tokens = getPropertyNameTokens(finalPath);
			PropertyHandler ph = nestedBw.getLocalPropertyHandler(tokens.actualName);
			if (ph != null) {
				if (tokens.keys != null) {
	 ...
	 */
	private String getFinalPath(A pa, String nestedPath) {
		if (pa == this) {
			return nestedPath;
		}
	...
	protected A getPropertyAccessorForPropertyPath(String propertyPath) {
		int pos = PropertyAccessorUtils.getFirstNestedPropertySeparatorIndex(propertyPath);
		// Handle nested properties recursively.
		if (pos > -1) {
			String nestedProperty = propertyPath.substring(0, pos);
			String nestedPath = propertyPath.substring(pos + 1);
			AbstractSmartPropertyAccessor<?> nestedPa = getNestedPropertyAccessor(nestedProperty);
			return (A) nestedPa.getPropertyAccessorForPropertyPath(nestedPath);
		}
		else {
			return self();
		}
	 ...
	 */
	private A getNestedPropertyAccessor(String nestedProperty) {
		if (this.nestedPropertyAccessors == null) {
			this.nestedPropertyAccessors = new HashMap<String, A>();
		}
		...
		A nestedPa = this.nestedPropertyAccessors.get(canonicalName);
	 ...
	 */
	protected abstract A newNestedPropertyAccessor(Object object, String nestedPath);
	...
	public void setPropertyValue(PropertyValue pv) throws BeansException {
		...
		if (tokens == null) {
			...
			A nestedBw;
			try {
				nestedBw = getPropertyAccessorForPropertyPath(propertyName);
			}
			catch (NotReadablePropertyException ex) {
				throw new NotWritablePropertyException(getRootClass(), this.nestedPath + propertyName,
						"Nested property in path '" + propertyName + "' does not exist", ex);
	...
	protected abstract boolean isOptional(PropertyTokenHolder tokens, PropertyValue pv);
