...
class DisposableBeanAdapter implements DisposableBean, Runnable, Serializable {
	 ...
	 */
	@Nullable
	static String[] inferDestroyMethodsIfNecessary(Class<?> beanType, RootBeanDefinition beanDefinition) {
		...
		if (destroyMethodName == null) {
			...
			boolean autoCloseable = (AutoCloseable.class.isAssignableFrom(beanType));
			if (AbstractBeanDefinition.INFER_METHOD.equals(destroyMethodName) ||
				...
				if (!(DisposableBean.class.isAssignableFrom(beanType))) {
					if (autoCloseable) {
						...
						try {
							destroyMethodName = beanType.getMethod(CLOSE_METHOD_NAME).getName();
						}
						catch (NoSuchMethodException ex) {
							try {
								destroyMethodName = beanType.getMethod(SHUTDOWN_METHOD_NAME).getName();
							}
